...
**Part 1 Answer:**
```
Starting: [8, 3, 5, 1, 9, 2]
After 3:  [3, 8, 5, 1, 9, 2]
After 5:  [3, 5, 8, 1, 9, 2]
After 1:  [1, 3, 5, 8, 9, 2]
After 9:  [1, 3, 5, 8, 9, 2]  (no change)
After 2:  [1, 2, 3, 5, 8, 9]
```
...
**Part 3 Answer:**
1. `range(0, len(arr))` should be `range(1, len(arr))`
2. `j = i + 1` should be `j = i - 1`
3. `arr[j] = arr[j + 1]` should be `arr[j + 1] = arr[j]`
4. `j = j + 1` should be `j = j - 1`
...
**Part 6 Answer:**
1. Zero times. The while loop only runs when we need to move items.
2. 10 times total (4+3+2+1).
3. Because the first item (index 0) is already considered sorted by itself.
